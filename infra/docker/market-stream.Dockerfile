...
#
# services/market-stream -- background worker (CLAUDE.md section 11/26/58).
#
# Maintains authoritative real-time market state: consumes market-scanner's
# lifecycle events, streams Polymarket order-book/trade data and an
# independent Coinbase underlying-price feed, normalizes both into
# Redis (for WebSocket fanout) and TimescaleDB (for history/backtesting).
...
#
#   docker build -f infra/docker/market-stream.Dockerfile -t grokpulse/market-stream .
#
# --- Why this shape ---------------------------------------------------------
# 1. `pruner`  : uses `turbo prune` to compute the minimal subset of this
#                pnpm/turborepo workspace needed to build @grokpulse/market-
#                stream (itself + its transitive workspace deps only).
...
# See infra/docker/market-scanner.Dockerfile for the pattern this mirrors;
# it is the one actually verified to build in this repo's CI sandbox
...
COPY . .
RUN turbo prune @grokpulse/market-stream --docker
...
RUN pnpm turbo run build --filter=@grokpulse/market-stream...
...
USER node
CMD ["node", "services/market-stream/dist/index.js"]
